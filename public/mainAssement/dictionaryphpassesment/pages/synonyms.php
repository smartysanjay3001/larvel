...
if(isset($_POST['submit'])){
 ...
 $wordid= $_SESSION['wordid'];
  ...
  $sql="select synonyms from Dictionary_words where word_id='$wordid'";

  $result=$conn->query($sql);
  if($result->num_rows >0){
    while($row=$result->fetch_assoc()){
      $array_sys=json_decode($row['synonyms'],true);

    }

    array_push($array_sys,$synonyms);
    $sys_array=json_encode($array_sys,true);
    $sql1="update Dictionary_words set synonyms='$sys_array' where word_id='$wordid'";
...
<div class="container ff sd">
        ...
        <h3 class="text-warning mx-auto ">Add Synonyms Here</h3>
        <input type="text" class="fff  py-2 px-3 rounded    w-auto " name="sys" >
